...
function fixture(cached=null){
 const writes=[];const callbacks=[];const elements={appShell:{style:{}},activationFeedbackMsg:{style:{}}};const user={uid:'u',email:'u@example.test',getIdToken:async()=> 'test-token'};
 ...
 const ctx={window,URL,fetch:async()=>{throw Error('The free release must not call a billing backend');},document:{readyState:'loading',addEventListener(){},getElementById:id=>elements[id]||null},console,localStorage:{removeItem(){}},sessionStorage:{getItem:()=>null}};
...
test('manual activation check refreshes the server profile instead of reusing the free cache',async()=>{
 ...
 await f.auth.refreshActivation();assert.equal(f.auth.getSubscription().plan,'pro');assert.equal(pushes,1);
 assert.match(f.elements.activationFeedbackMsg.textContent,/membership is active/);
});
test('signing out during the activation check cannot reopen the old account',async()=>{
 const f=fixture({subscription:{plan:'free',limit:20}});await f.signIn();
 ...
 const redeem=f.auth.refreshActivation();await pending;await f.signOut();
...
test('manual support includes the current account and never grants membership',async()=>{
 const f=fixture({subscription:{plan:'free',limit:20}});await f.signIn();let opened;
 f.window.open=(...args)=>opened=args;f.auth.handleActivationSupport();
 const url=new URL(opened[0]);assert.equal(url.hostname,'wa.me');assert.match(url.searchParams.get('text'),/User ID: u/);
 assert.equal(opened[2],'noopener,noreferrer');assert.equal(f.auth.getSubscription().plan,'free');assert.equal(f.writes.length,0);
});
test('checking an unapproved account reports pending without changing its plan',async()=>{
 const f=fixture({subscription:{plan:'free',limit:20}});await f.signIn();await f.auth.refreshActivation();
 assert.match(f.elements.activationFeedbackMsg.textContent,/pending/);assert.equal(f.auth.getSubscription().plan,'free');assert.equal(f.writes.length,0);
});
